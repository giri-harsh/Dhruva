...
| | |
|---|---|
| **Version** | **3.0 — FINAL. The definitive, canonical specification. Supersedes v1.0, v1.1 (`PRD.md`), v1.2 (`PRD kamal.md`), v2.0 (`ANCHOR-PRD-v2.0.md`) and the Dhruva brief. This is the single document the team builds from.** |
...
### What changed in v3.0, and why this is the last merge the team needs to do

**v3.0 is a consolidation, not a rewrite.** Three PRDs were circulating (`PRD.md` v1.1, `PRD kamal.md` v1.2, `ANCHOR-PRD-v2.0.md`) and a team cannot build against three documents at once. v2.0 remains the technical and evidentiary base — its architecture, FR list, risk register and evaluation discipline are carried forward unchanged except where noted below. v3.0 adds exactly four things, each closing a gap the hackathon brief calls out explicitly, and folds in the one piece of v1.2's execution hardening that v2.0 had only referenced in passing.

| # | Change | Forced by |
|---|---|---|
| **W1** | **Consolidation.** Three source documents (v1.1, v1.2, v2.0) merged into one canonical file. v1.2's execution-hardening content was checked against v2.0 line by line; everything not already superseded (Kotlin core, the pivot table already in §20.1, DPDP timing already in §9.5/Appendix B, CI hard-blocks already in Appendix A) was either already present or is folded in below. The one genuinely new piece of content — full corridor-recording logistics — is added to §19.3. | Team needed one document, not three |
| **W2** | **A Python backend services tier, and the architecture restated explicitly as three tiers.** The on-device engine stays Kotlin/JVM — that decision (§10.2) is unchanged and correct. But model *training*, the flywheel *retraining* loop, the model registry, telemetry ingestion and the evaluation dashboard's API now have a named home: a Python/FastAPI backend, which is also where "AI/ML" work is most naturally read as happening by a judge or a reviewer skimming the stack. §10 is rewritten around this. | Team requirement: "a python backend easier to integrate with ML AI" |
| **W3** | **A moat/competitor callout pulled to the front of §4**, answering directly, in one table, what a judge otherwise has to extract from prose: what this saves, and what is different from Google/Apple Maps, MapmyIndia, OEM INS, OBD dongles, WhONet, AI-IMU and AVNet. | Judging-criteria legibility |
| **W4** | **An explicit "which USP is the wow" framing sentence added to §5**, and **a fourth SDG (SDG 8, decent work) added to §16.1**, grounded in the existing Ravi persona (§7.1–7.2) rather than asserted new. | Judging-criteria legibility; societal-impact completeness |

**Unchanged from v2.0, deliberately:** the Kotlin/JVM core decision (§10.2), all 34 functional requirements and their acceptance criteria (§8), the risk register (§18), the roadmap and Week-5 pivot table (§20, §20.1), the demo script's core sequence (§19), and every fact-check and novelty-scoring appendix (E, F). **No v2.0 correction is reintroduced or reversed** — the novelty claim stays narrowed and AVNet-cited, the filter stays ESKF (not UKF), the GNSS-trust component stays the chi-square gate (not `GnssTrustNet`), and the core stays Kotlin (not C++).

<details>
<summary>What changed in v2.0 (superseded by, but preserved as history in, the paragraph above)</summary>

Six changes, each forced by evidence rather than preference, forced the move from v1.2 to v2.0. **The fact-check that forced them is Appendix E**; the prior-art search behind V1 and V3 is Appendix F.
...
### 4.0 The one-table version, for a judge with thirty seconds

Every one of the questions below is answered in full elsewhere in this section and in §2. This table exists because a judge should not have to extract the answer from prose.

| Question | Answer |
|---|---|
| **Is this saving time, cost, or efficiency?** | **Time**, primarily — §7.2's Ravi journey recovers an estimated 3–6 minutes per covered-area delivery `[VERIFY]`, and §16.2 shows every transit of one named tunnel currently loses 9.3 minutes of navigation entirely. **Cost** follows structurally, not as a separate feature: §4.2 shows marginal positioning cost is ₹0 per vehicle because inference is on-device — there is no server in the loop to pay for, ever. |
| **vs Google / Apple Maps / MapmyIndia** | They have the phone but not the physics: excellent when a GNSS fix exists, but the marker freezes or teleports the moment it does not, with no exposed confidence and no way for a fleet to audit or tune the black box. (§2) |
| **vs OEM/factory INS (u-blox-class DR chipsets)** | They have the physics but not the phone: genuinely solves the problem, but needs a hardware install and a wheel-tick/CAN feed — absent from most Indian commercial vehicles and every two-wheeler. (§2) |
| **vs OBD-II dongles** | Needs a port and a dongle that stays plugged in; no OBD-II on two-wheelers; explicitly ruled out by the PS itself. (§2) |
| **vs the closest published research (WhONet, AI-IMU, AVNet)** | They have the rigour but assume a sensor the target vehicle does not have (WhONet: wheel encoders; AI-IMU: a rigidly-bolted 100 Hz automotive IMU) — **or**, in AVNet's case, they already do the phone-only substitution and beat our threshold by ~15×, which we cite rather than compete with. What none of the three ships: a *calibrated* uncertainty fed to the filter, explicit refusal behaviour, a road-manifold constraint, a measured integrity curve, or an identical engine on a phone and a 200 Hz industrial IMU. (§2.1, Appendix F.5) |
| **What is actually defensible** | Half the system (the ESKF, NHC/ZUPT, HMM map matching, even the velocity-head *concept*) is textbook or already published — no moat, and we say so. What is not clonable in a week is the leakage-safe training protocol, the calibrated-uncertainty discipline, and the data + magnetic flywheel — the only one of the three that grows over time. (§4.4) |
...
**Which one is the actual wow.** Read individually these are five features. Read as one claim, they are a single wow-factor: **this is proven live, not claimed on a slide.** Every other team's "AI-based sensor fusion" is a diagram; ours is a phone in a judge's own hand, tracking through a corridor with location services switched off by the judge themselves, showing a live number for how wrong it is instead of pretending to be exact. The multi-signal fusion (learned velocity + road-manifold constraint + magnetic memory + GNSS integrity gate) is what makes the number defensible; **the judge-operated demo is what makes it believed.** Neither half works alone: fusion without a live demo is a paper; a live demo without the fusion underneath would not survive a tunnel.
...
### 10.0 Three tiers, stated plainly for a judge in ten seconds

**New in v3.0: the architecture is restated explicitly as three tiers, because a judge should be able to place every component in one of three boxes without reading the rest of this section.**

| Tier | What lives here | Technology |
|---|---|---|
| **1 — Presentation** | The Android app's UI (map, marker, confidence ring, mode pill), and the edge/CLI engine's terminal output or log stream. Renders only — no business rule, no repository access. | Kotlin (Jetpack Compose) for Android; plain stdout/log for the CLI |
| **2 — Application / business logic** | Split into two halves that never share process space but share one contract. **(a) The on-device positioning engine** — sensor fusion, the 15-state ESKF, NHC/ZUPT, map matching, the road-manifold constraint, magnetic memory, the chi-square gate. **Must run fully offline**; this half never assumes a network exists. **(b) The Python backend services** — the API, the retraining pipeline, orchestration and evaluation logic. This half never touches a live position; it exists entirely outside the positioning loop. | (a) Kotlin/JVM, one artefact, §10.2 · (b) Python 3 / FastAPI, §10.2b |
| **3 — Data** | Offline OSM map extracts (on-device and in the CDN origin), the model registry / artefact store, the opt-in label-pair and magnetic-signature stores, the golden evaluation dataset, and the trip/telemetry database. | On-device: SQLite/Room, memory-mapped `.pbf`. Server-side: Postgres + object storage, behind the same repository-interface discipline as §10.1's dependency rule. |

**Why the Application tier is drawn as two halves rather than one box.** They have opposite constraints and neither can compromise the other. The on-device half must work with **zero network access for the entire trip** (FR-22) — it is where PS 26168's actual requirement lives, and it is unconditionally offline. The backend half's entire reason to exist is the opposite: it is where *connectivity is assumed* and where the flywheel, retraining and distribution logic that keeps the on-device half improving actually runs. Putting both under one "Application" tier, rather than treating the backend as an appendage bolted outside the architecture, is what makes the three-tier framing accurate rather than decorative — the backend is not infrastructure plumbing, it is where the system's *learning* half of "AI/ML" lives, deliberately separated from the *inference* half that must never depend on it.

```mermaid
flowchart TB
    subgraph T1["TIER 1 — PRESENTATION"]
        direction LR
        A1["Android app<br/>(Kotlin / Compose)"]
        A2["Edge / CLI engine<br/>log + terminal UI"]
        A3["Web eval dashboard<br/>(React/served by backend)"]
    end
    subgraph T2["TIER 2 — APPLICATION / BUSINESS LOGIC"]
        direction LR
        subgraph T2A["2a — on-device engine (Kotlin/JVM)<br/>MUST run fully offline"]
            B1["Sensor fusion · ESKF<br/>NHC · ZUPT · chi-square gate"]
            B2["Road-manifold constraint<br/>Magnetic memory · Map matching"]
        end
        subgraph T2B["2b — Python backend services (FastAPI)<br/>assumes connectivity"]
            C1["Model registry & OTA push"]
            C2["Flywheel retraining pipeline<br/>(PyTorch)"]
            C3["Telemetry ingestion API"]
            C4["Evaluation dashboard API"]
        end
    end
    subgraph T3["TIER 3 — DATA"]
        direction LR
        D1["OSM map extracts"]
        D2["Model artefact store"]
        D3["Label-pair / mag-signature store<br/>(opt-in)"]
        D4["Golden eval set"]
        D5["Trip / telemetry DB"]
    end

    T1 --> T2A
    T1 --> T2B
    T2A -.->|"exported .onnx, pulled OTA"| C1
    T2A -.->|"opt-in upload only"| C3
    T2B --> T3
    T2A -.->|"local reads only, no network"| D1
    ...
    class A1,A2,A3 pres
    class B1,B2,C1,C2,C3,C4 app
    class D1,D2,D3,D4,D5 data
```

> ### In plain terms
> Three layers: what you see, what decides, and what is stored. The middle layer is split into two halves that deliberately never depend on each other at runtime — one half runs on the phone and has to work with no internet at all, because that is the entire point of the product; the other half runs on a server in Python, and it is where the model gets *trained* and *retrained* on real driving data, where new models get pushed out to phones, and where the evaluation dashboard gets its numbers. The phone never needs the server to know where it is. The server exists so that, over time, the phone gets better at knowing where it is.
...
### 10.2b The Python backend — where the "AI/ML" in "AI/ML-based" actually happens

**New in v3.0.** The Kotlin decision in §10.2 is about the on-device *inference* artefact — the compiled thing that turns a window of sensor data into a displacement and a variance in single-digit milliseconds, with no Python runtime anywhere near it. It is deliberately not about where the model is *trained*, where it is *retrained* on new driving data, or where the team's evaluation and telemetry tooling lives. **Those are a separate concern with a separate correct tool, and v1.x/v2.0 left this implicit rather than naming it as a tier.** v3.0 names it: a Python backend, built on **FastAPI**, is Tier 2b of §10.0, responsible for exactly five things and nothing else.

| Responsibility | What it does | Why Python here, specifically |
|---|---|---|
| **Model training & the flywheel retraining pipeline** | PyTorch training loop (§14.3) run on committed split manifests; ingests opt-in `LABEL_PAIR` and `MAG_SIGNATURE` rows from `/labels/batch` and `/magsig/batch` (§13); re-trains `ANCHOR-Net` on the growing Indian-road distribution; re-runs the golden-set gate (§14.7) before any retrained model is allowed to ship. | This is squarely where "AI/ML" work belongs — PyTorch, NumPy, the training and evaluation stack already specified in §14 and `ml/` (§10.5) run here natively, with no cross-language step. |
| **Offline map-extract hosting and distribution** | Builds and serves the `.osm.pbf` region extracts (`/maps/regions`, §13) that every device downloads quarterly; versions and checksums them into `MAP_EXTRACT` (§12.1). | A straightforward CDN-fronted static-asset service; FastAPI's job here is thin routing and signed-URL issuance, not computation. |
| **Model registry and OTA model-push** | `ModelRepository`'s server side: signs each exported `.onnx` artefact (`ml/export/sign_manifest.py`), serves `/models/latest`, and enforces `min_app_version` gating (§13, FR-25). | Same signing and versioning pipeline that produces the model in the first place; keeping registry logic next to training logic means a retrained model can be gated on the golden set and published in one pipeline run, not handed across a language boundary. |
| **Evaluation / replay web dashboard's API** | Serves the plots, ablation table, calibration reliability diagram and integrity ROC (§14.6, §14.7, FR-31) that `web/` (§10.5) renders — the same artefact submitted for proposal screening. | The dashboard's data *is* the ML pipeline's output; serving it from the same Python process that produced it avoids a redundant export/reformat step. |
| **Telemetry ingestion** | `/labels/batch`, `/magsig/batch` and the fleet quality endpoints (`/fleet/trips`, `/fleet/trips/{id}/quality`, §13) — validated against the OpenAPI schema, bounds-checked (§12.2), consent-checked server-side (FR-23's `403`), and rate-limited (§13). | Ordinary CRUD-and-validation API work; FastAPI's OpenAPI-first design is exactly the contract-enforcement mechanism §13 already specifies. |

**What the Python backend is explicitly *not*.** It never computes a position, never sees a live trip in progress, and is never in the critical path of §9.1's latency budget — those are Tier 2a's job, entirely, on-device, offline. **FR-22's "zero network access for the entire trip" is unchanged by this section**; adding a named backend tier does not add a dependency the on-device engine has on it. The relationship is one-directional and asynchronous: the backend trains a model over hours or days from accumulated opt-in data, signs it, and a device *pulls* it whenever it next has connectivity — never the reverse, and never inline with a positioning update.

**Deployment shape.** `backend/` is a single FastAPI application (Uvicorn/Gunicorn), backed by Postgres (trip/fleet/consent metadata) and object storage (map extracts, model artefacts, quantised label/signature blobs) — the same components §9.2's load figures were already sized against. `ml/` remains the training/evaluation *library*; `backend/` is the thin service layer that schedules and exposes it. This is a refactor of where existing responsibilities are named, not a new capability: every endpoint in §13's API contract, the retraining loop in §14.3, and the dashboard in S-17 already existed in v2.0 — v3.0 states, once and explicitly, that all of them are the same Python service.

> ### In plain terms
> The phone never runs Python and never needs to — the thing making a position decision at highway speed is a small compiled Kotlin program with no network call in it anywhere. But behind the scenes, on a server, there is a Python service doing everything people usually mean by "AI/ML": training the model on PyTorch, retraining it as more (opted-in) driving data comes in, checking a retrained model against the forty held-out recordings before it's allowed to ship, and serving the dashboard of plots a judge or a fleet manager would look at. The phone downloads the finished, compact result of that work every few months. It never depends on the server being reachable to know where it is right now.
...
flowchart TB
    ...
    subgraph EXT["TIER 2b — PYTHON BACKEND SERVICES (FastAPI) — outside the device, assumes connectivity"]
        CDN["Map extract CDN"]
        API["Label + magsig ingestion API<br/>(opt-in, §13)"]
        TRAIN["Training + flywheel<br/>retraining pipeline<br/>PyTorch · IO-VNBD"]
        REG["Model registry<br/>+ OTA push (FR-25)"]
        BENCH["Integrity bench (FR-31)<br/>attack injector + ROC"]
        DASH["Eval dashboard API<br/>(serves web/, S-17)"]
    end
    ...
    R6 -.-> API
    R7 -.-> API
    TRAIN -.->|exports .onnx| REG
    REG -.->|signed manifest| R1
    BENCH -.-> SS
    UI5 --> DASH
    DASH --> TRAIN
    DASH --> BENCH
    ...
    class CDN,API,TRAIN,REG,BENCH,DASH ext
```

**Reading this against §10.0's three-tier statement.** Everything inside `ENG` (`:core`) plus `VAL`/`SVC` is **Tier 2a**. Everything inside `EXT` — `CDN`, `API`, `TRAIN`, `REG`, `BENCH`, `DASH` — is **Tier 2b**, and it is one Python/FastAPI service, not five separate systems; it is drawn as five nodes here only because each has a distinct responsibility from §10.2b's table. `PRES` is Tier 1. `DATA` is Tier 3. No arrow crosses from `EXT` into the live positioning path (`ORCH`→`SS`→…→`MM`) — the only connections are the OTA model pull (`REG -.-> R1`, consumed asynchronously by `MODEL` the next time the device loads a manifest) and the opt-in uploads (`R6 -.-> API`, `R7 -.-> API`), both of which are §10.0's "backend is not in the loop" claim, drawn.
...
│                              (a thin client; its data comes from backend/)
├── backend/                   TIER 2b — Python/FastAPI. Never in the positioning loop.
│   ├── app/
│   │   ├── main.py             FastAPI app, OpenAPI 3.1 spec is the source of truth
│   │   ├── routers/            maps.py · models.py · labels.py · magsig.py · fleet.py
│   │   ├── registry/           Model signing, versioning, min_app_version gating (FR-25)
│   │   ├── ingestion/          /labels/batch, /magsig/batch — bounds + consent checks
│   │   ├── dashboard_api/      Serves web/'s plots, ablations, calibration, ROC
│   │   └── db/                 Postgres models: trip/fleet/consent metadata (mirrors §12)
│   └── tests/                  Contract tests generated from openapi.yaml; cross-tenant
│                                403 asserted on every /fleet/* route (T5, §15.1)
├── maps/                      OSM extract build scripts + checksums. No .pbf in git.
└── docs/                      ADRs, threat model, DPDP notes, demo script, runbooks
```

**Note on `ml/` vs `backend/`.** `ml/` (above) is the training and evaluation *library* — the code that knows how to train `ANCHOR-Net`, run the ablations, and score the integrity bench. `backend/` is the thin FastAPI *service* that schedules `ml/`'s pipeline, serves its outputs over HTTP, and owns the registry and ingestion contract in §13. The split mirrors the difference between a Python package and a Python service — `ml/` has no HTTP surface and can be run from a notebook or a CI job; `backend/` imports `ml/` and exposes it. Neither is shipped to a device.
...
**There is no positioning API.** Position is computed on the device and never leaves it. The server surface is deliberately tiny: map distribution, model distribution, and opt-in ingestion — implemented as the Python/FastAPI backend of §10.2b (Tier 2b), never in the positioning loop.
...
| **SDG 8 — Decent Work and Economic Growth** *(added in v3.0)* | **8.5** — *"achieve full and productive employment and decent work for all … including for … persons with disabilities, and equal pay for work of equal value"* — read here for its "decent work" clause, applied to gig and platform work | This is Ravi's persona, stated already in §1.3 and §7.1–7.2, not a new claim: he is **paid per delivery and penalised on SLA** (§7.1), and §7.2's as-is/to-be table estimates **~3–6 minutes recovered per covered-area delivery** `[VERIFY]`. For a rider paid per completed delivery, minutes not lost to a frozen map inside a basement or underpass convert directly into more completed deliveries per shift and fewer SLA penalties — i.e. **better effective earnings for the same hours worked**, with no change to platform policy required. | Minutes recovered per shift × deliveries completed per shift, from opt-in trip telemetry (`mode_event`, `pose`, §12) on a partner fleet — the same measurement infrastructure §16.2's beneficiary estimate already requires. **We do not claim a wage increase** — only the time mechanism, which is the honest and measurable half of the claim. |

**A note on honesty for SDG 3 and SDG 8.** We can measure the *mechanism* in both cases — fewer glances at a frozen screen, faster emergency routing, minutes recovered per shift. **We cannot claim a causal reduction in road deaths, and we cannot claim a wage increase, from a hackathon project, and we will not put either claim on a slide.** Stating the mechanism and the proxy indicator, and stopping there, is the correct scope for both.
...
### 19.2 Corridor-recording logistics — pulled forward from v1.2, still not closed by anything above

**Folded in from `PRD kamal.md`'s Appendix D3, which v2.0 reduced to one Appendix B line ("corridor-recording permission and a named backup corridor + backup date, Week 5") without restating the reasoning.** §5.1 and §16.2's entire evidentiary case depends on a real recorded run through an actual GNSS-denied corridor. That is a **logistics** question — permission, safety, a backup date — not a data question, and it deserves more than a `[VERIFY]` tag.

- **Prioritise the accessible target first.** A covered multi-level car park is far easier to get permission for than a national highway tunnel — prioritise it as the primary recording target. The NH-44 Chenani–Nashri figure in §1.4/§16.2 remains the *arithmetic example* in the pitch regardless of which corridor is actually filmed; the two are not required to be the same location.
- **Get written permission before Week 7.** Facility management for a car park; NHIDCL or the local highway authority if a tunnel is attempted. Recording without it risks losing the primary demo asset for reasons that have nothing to do with the engineering.
- **Record in Week 6–7, not Week 8**, so there is one full week of buffer for a re-shoot if the first recording is unusable (bad GPS lock beforehand, phone mount failure, traffic, a permission delay).
...
- **Owner:** the generalist, starting no later than Week 5 (already reflected in Appendix B). This subsection is the reasoning Appendix B's one-line entry was shorthand for.

### 19.3 What writing the demo first changed about scope
...
| **Societal impact** | **The exclusion, not the total.** One line: *"21.42 million two-wheelers registered in FY26. None has an OBD-II port."* SDG 9.1 / 11.2 / 3.6 / 8.5 listed by number beneath. | *"We're not claiming to help hundreds of millions of people — we're pointing at the specific population that physically cannot be helped any other way, because there is no port on a scooter to plug anything into."* |
...
**v2.0 → v3.0 (this document, FINAL):**

| # | Change | Why | Sections |
|---|---|---|---|
| **W1** | **Consolidation of three source PRDs into one canonical document.** `PRD.md` (v1.1) and `PRD kamal.md` (v1.2) checked against v2.0 line by line; everything not superseded is folded in. | A team cannot build against three documents | Whole document; header table |
| **W2** | **Python/FastAPI backend services tier named explicitly (§10.0, §10.2b), and the architecture restated as three tiers** — Presentation, Application (split into the offline Kotlin on-device engine and the connectivity-assuming Python backend), Data. | Team requirement: "a python backend easier to integrate with ML AI"; also sharpens judging-panel legibility of the stack | §10 (new §10.0, §10.2b), §13, component diagram, repo tree |
| **W3** | **Moat/competitor summary table pulled to the front of §4** (§4.0), answering time/cost/efficiency and the six-way competitive comparison in one place rather than leaving it distributed across §2 and §4. | Judging-criteria legibility | §4.0 |
| **W4** | **Primary-wow framing sentence added to §5**; **SDG 8 (decent work) added to §16.1**, grounded in the existing Ravi persona and §7.2's time-recovery estimate, not asserted new. | Judging-criteria completeness; the brief named both explicitly | §5, §16.1, §21 |
| **W5** | **Corridor-recording logistics restored to full detail (§19.2)**, folded in from `PRD kamal.md`'s Appendix D3 — v2.0 had compressed it to a single Appendix B line without the underlying reasoning. | v1.2 content that was genuinely additive and had been trimmed too far | §19.2 |

**Checked and confirmed unchanged from v2.0, per the reconciliation this consolidation required:** the Kotlin/JVM on-device core (§10.2) — the Python backend added in W2 is a new *named tier for existing responsibilities*, not a reversal of the C++→Kotlin decision, and does not touch the device; the classical error-state EKF (not UKF); the chi-square innovation gate (not `GnssTrustNet`); ZUPT as a first-class filter update (FR-26). **This matches the team's "Approach A" architecture decision as previously recorded, with no inconsistency found.**
...
*End of PRD v3.0 — FINAL. The canonical, combined specification, superseding `PRD.md` (v1.1), `PRD kamal.md` (v1.2) and `ANCHOR-PRD-v2.0.md` in full. Every quantitative claim in this document is either linked to a source or listed in Appendix B. This document is self-contained: no companion file is required to read, review or submit it.*
